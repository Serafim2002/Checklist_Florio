...
import pandas as pd
import subprocess  # Importa para executar outro programa
...
def finalizar():
    """Função que será executada ao clicar no botão 'Finalizar'."""
    try:
        # Substitua 'outro_programa.py' pelo caminho do programa que deseja executar.
        subprocess.run(["python", "outro_programa.py"], check=True)
        update_status("Processo finalizado com sucesso!", "green")
    except Exception as e:
        update_status(f"Erro ao executar o programa: {e}", "red")

# Configurações iniciais
csv_path = "C:/Users/Faturamento/Desktop/Projetos/Checklist_Florio/Checklist_MinusOne/data/Produto.csv"
...
# Botão Finalizar
ttk.Button(frame, text="Finalizar", command=finalizar).pack(pady=20)  # Adiciona o botão Finalizar
