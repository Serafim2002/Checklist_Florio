...
class PedidoJanela:
    ...
    def criar_pedido(self):
        pedido = self.texto_pedido.get("1.0", "end-1c")
...
class QRCodeProcessor:
    ...
    def processar_qrcode(self):
        ...
        with open(self.arquivo_entrada, "r") as arquivo_entrada:
            linhas = arquivo_entrada.readlines()

        linhas_processadas = [self.processar_linha(linha) for linha in linhas]

        with open(self.arquivo_saida, "w") as arquivo_saida:
    ...
    def processar_linha(self, linha):
        ...
        if len(partes) == 3:
            Volume = partes[0]
            descricao_produto = partes[1].strip() + " " + partes[2].strip()
